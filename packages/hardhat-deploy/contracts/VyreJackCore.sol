...
contract VyreJackCore is IVyreGame, IVRFConsumer, Initializable, UUPSUpgradeable {
    ...
    /// @notice VRF Coordinator for randomness (not immutable for upgradeability)
    ...
    // ----------------------------------------------------------------------
    // ░░  INITIALIZER (UUPS Pattern - replaces constructor)
    // ----------------------------------------------------------------------
    ...
    /**
     * @notice Initializes VyreJackCore with VRF and Casino configuration.
     * @dev Can only be called once via proxy. Use reinitialize() for upgrades.
    ...
    function play(
        ...
        vrfRequests[requestId] =
            VRFRequest({ player: player, requestType: RequestType.InitialDeal, fulfilled: false, timestamp: block.timestamp });
    ...
    function hit() external {
        ...
        vrfRequests[requestId] = VRFRequest({
            player: msg.sender, requestType: RequestType.PlayerHit, fulfilled: false, timestamp: block.timestamp
        });
    ...
    function double() external {
        ...
        vrfRequests[requestId] = VRFRequest({
            player: msg.sender, requestType: RequestType.PlayerDouble, fulfilled: false, timestamp: block.timestamp
        });
    ...
    /// @notice Handle double down card - take one card then auto-stand to dealer
    ...
    function _playDealer(
        ...
        if (cardsNeeded > 0) {
            ...
            vrfRequests[requestId] = VRFRequest({
                player: player, requestType: RequestType.DealerDraw, fulfilled: false, timestamp: block.timestamp
            });
        } else {
            _resolveGame(player);
        }
    ...
    /**
     * @notice Authorizes contract upgrades (required by UUPS)
    ...
    function fallbackCommit(
        ...
        require(
            block.timestamp > game.timestamp + VRF_TIMEOUT,
            "VyreJackCore: VRF not timed out"
        );
    ...
    function fallbackReveal(
        ...
        require(
            keccak256(abi.encodePacked(secret)) == req.commitment,
            "VyreJackCore: invalid reveal"
        );
        require(
            block.number > req.blockNumber,
            "VyreJackCore: wait one block"
        );
    ...
    function getVRFRequest(
        uint256 requestId
    ) external view returns (address player, RequestType requestType, bool fulfilled, uint256 timestamp) {
